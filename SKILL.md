...
FireSeed 已集成 [MarkdownFlow](https://markdownflow.ai) — 一种在 Markdown 正文中直接写交互元素的语法。

### 在章节正文中使用

作者在章节正文中写入以下语法，读者端自动渲染为交互元素：
...
| 语法 | 用途 | 示例 |
|------|------|------|
| `{{变量}}` | 变量替换 | `{{reader_name}}` → 当前登录读者名 |
| `?[A \| B \| C]` | 单选按钮 | `?[红色 \| 蓝色 \| 绿色]` |
| `?[A \|\| B \|\| C]` | 多选 | `?[Python \|\| JS \|\| Go]` |
| `?[%{{var}} A \| B]` | 存储到变量 | `?[%{{choice}} 去 \| 留]` |
| `?[...输入提示]` | 文本输入 | `?[...输入你的名字]` |
| `A//val` | 按钮ID | `?[确认//confirm \| 取消//cancel]` |
| `===保留内容===` | 锁定内容 | `这是===神圣誓言===` |
...
选择后 → 自动跳转到对应分支章节。和 `choices` JSON 字段效果相同，但更直观。
...
- **Token 有效期 7 天**，过期后需重新登录获取（旧版为 30 天）
