...
- 如果不存在，提示用户提供用户名和邮箱和密码，然后自动调用注册/登录接口获取 Token，并保存到配置（`openclaw config set fireseed.token <token>`）。
...
| 版本 | 日期 | 更新内容 |
|------|------|----------|
| **v3.6.1** | 2026-06-11 | 平台新增：邮件通知系统（注册欢迎邮件+玩法指南）、更新通知API、数据库自动备份、AI跑团优化、可配置LLM端点 |
