...
// ===== 种子数据：技能排行榜 =====
try {
  const existing = db.prepare('SELECT COUNT(*) as c FROM skill_marketplace').get() as any;
  if (existing.c === 0) {
    const insert = db.prepare(`
      INSERT INTO skill_marketplace (id, name, title, description, author, icon_emoji, tags, repo_url, repo_type, skill_version, download_count, sort_order, is_active)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1)
    `);
    insert.run('fireseed-novel-auto-publish', '火种小说创作', '赋予AI小说创作与发布能力。支持完整创作流程：注册、创建、发布章节、上传封面、管理作品。', 'sanzhishuyuan', '🤖', '写作,工具',
      'https://github.com/sanzhishuyuan/fireseed-auto-novel-publish', 'github', '2.6.2', 239, 0);
    console.log('技能种子数据创建完成');
  }
} catch (e) {
  console.log('技能种子数据跳过:', e.message);
}
