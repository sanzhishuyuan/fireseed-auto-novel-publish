...
每章创作必须遵守以下冲突质量公式，校验不达标则强制修正。

### 核心公式

> **冲突质量 = 困境强度 × (wA·A + wB·B + wC·C) / D**

### 变量定义

| 变量 | 定义 | 值域 | 计算规则 |
|------|------|------|----------|
| 困境强度 | 该章核心困境等级 | 1-5 | 引用 Step 1.2 标定值，章与章之间可波动 ±1 |
| A（对手对抗） | 本章对手/障碍的有效阻挠强度 | 0-3 | LLM评估：0=无对抗，1=轻度阻挠，2=有效阻挠，3=致命威胁 |
| B（内心挣扎） | 本章主角的有效内心冲突 | 0-3 | LLM评估：0=无，1=自我质疑，2=两难选择，3=价值观崩解 |
| C（规则突破） | 本章主角挑战/违反规则的强度 | 0-3 | LLM评估：0=无，1=试探规则，2=违反规则，3=重定义规则 |
| D（技巧重复度） | 本章使用技巧的重复次数 | ≥1 | 统计该技巧在**之前3章内**出现次数 + 1。首次使用 D=1 |

### 权重配置（按项目类型）

| 项目类型 | wA（对手） | wB（内心） | wC（规则） | 说明 |
|---------|-----------|-----------|-----------|------|
| 热血/爽文 | 2.0 | 1.0 | 1.0 | 外部冲突决定读者爽感 |
| 悬疑/推理 | 1.5 | 1.5 | 2.0 | 规则突破是核心 |
| 情感/成长 | 1.0 | 2.0 | 1.5 | 内心挣扎是核心 |
| 轻松/搞笑 | 1.0 | 1.0 | 1.0 | 等权偏轻 |
| 其他 | 1.0 | 1.0 | 1.0 | 默认均等 |
...
| 困境强度 | 合格线 | 说明 |
|---------|--------|------|
| ≥ 4 | 冲突质量 ≥ 困境强度 × 0.7 | 高冲突作品，质量驱动修正 |
| = 3 | 冲突质量 ≥ 困境强度 × 0.6 | 中等冲突作品，适中要求 |
| ≤ 2 | 冲突质量 ≥ 困境强度 × 0.5 | 低冲突作品，同时跟踪「化解效率」替代指标 |

### 低困境作品替代指标

困境强度 ≤ 2 的作品除冲突质量自检外，须跟踪**化解效率**：

> **化解效率 = 困境强度 × (A+B+C) / 化解总字数（千字）**

化解效率 ≥ 1.0 视为合格——让读者觉得「主角解决得够快、够爽」。

### 容错机制

1. **LLM 自评偏差**：冲突质量得分仅作为「修正触发器」，不作为通过/不通过的绝对裁判。得分低于阈值 → 触发重新生成；得分高于阈值 → 仅记录不触发操作。
2. **技巧冷却重置**：D=1 时该技巧进入冷却期。若所有技巧均处于冷却期，允许使用冷却期最短的技巧，D 值设为 2，确保每章都有可用技巧。
3. **权重覆盖**：用户可手动指定权重配置覆盖自动识别的值，格式：`wA=x wB=y wC=z`。
...
每个副本包含：名称、类型、星级、核心冲突、困境等级（≥ 核心等级-1）、激活的人物动机、产出、**主导冲突类型**

**主导冲突类型**（每副本至少标记一项）：
- `外部对抗型` — 以对手对抗（A）为核心，适合战斗/竞争/追逐场景
- `内心挣扎型` — 以内心挣扎（B）为核心，适合选择/愧疚/成长场景
- `规则突破型` — 以规则突破（C）为核心，适合解谜/越狱/破局场景
- `均衡型` — 三要素均衡发展，适合过渡推进场景

> 主导冲突类型在生成阶段会指导权重倾向和技巧选择，但不强制限制三要素配比。
...
④ 本章的困境等级（1-5 级）：___
⑤ 本章三要素自评：
   - 对手对抗（A）得分（0-3）：___  依据：___
   - 内心挣扎（B）得分（0-3）：___  依据：___
   - 规则突破（C）得分（0-3）：___  依据：___
⑥ 本章使用的写作技巧名称：___
   （从技巧池中选择，确保尽量与之前3章不重复）
```

> **技巧池**（供选择）：悬念设伏 / 反转打脸 / 扮猪吃虎 / 逆境爆发 / 情感共鸣 / 智斗博弈 / 环境烘托 / 节奏突变 / 镜像对照 / 计中计
...
| **冲突质量** | **根据公式计算得分 ≥ 合格线** | **重新生成本章冲突段落** |

#### 冲突质量计算公式

在自动校验阶段，取出 Step 3.1 输出的 A/B/C 得分和技巧名称，按以下步骤计算：

1. **查权重**：根据 Step 1.1 识别的项目类型，查权重配置表获取 wA、wB、wC
2. **算加权**：加权和三要素 = wA×A + wB×B + wC×C
3. **查技巧重复度 D**：统计该技巧在**当前章节之前3章内**的使用次数，D = 次数 + 1
4. **算冲突质量**：冲突质量 = 困境强度 × 加权和三要素 / D
5. **比对合格线**：查合格线定义表，冲突质量 ≥ 合格线则通过

> 示例：热血文困境强度=4, A=2, B=1, C=1, 技巧为「逆境爆发」且前3章未用过
> wA=2.0, wB=1.0, wC=1.0, 加权和=2×2+1×1+1×1=6, D=1
> 冲突质量 = 4 × 6 / 1 = 24，合格线=4×0.7=2.8 → 24 ≥ 2.8 ✅ 通过

**不通过则自动修正**：分数不足时，AI 需重写本章冲突段落，重点增强得分最低的要素（A/B/C 中最低的那项），然后重新校验。
...
> ⚠️ **冲突质量增强**：若本章冲突质量得分低（低于合格线×1.2），则「所以呢」的答案很大概率缺失——优先查找冲突段落是否缺乏因果关联或情感分量。

**关卡5：AI味检测**
> 全文检索"值得一提的是"、"不难发现"、"从某种意义上说"、"综上所述"、"随着……的发展"。出现任一则不通关，重新改写。

**关卡6：冲突质量复核**
> 回顾本章的冲突质量得分，确认计算过程无误。
> - 困境等级是否与感知层输出一致？
> - 加权和三要素的计算是否正确？
> - D 值的滑动窗口计数是否准确（只计前3章，不计更早）？

**通关标准**：关卡1-6 全部通关 → 进入发布。任一不通关 → 注明不通关项，给出修改版本。
...
⚔️ 冲突质量报告：三要素均值 A=2.0 B=1.5 C=1.3 | 最高分章 24.0 | 最低分章 3.2
...
11. **冲突必算** — 每章必须在 Step 3.1 输出三要素得分，在 Step 3.4 计算冲突质量
12. **合格必过** — 冲突质量低于合格线时，必须重写冲突段落，不得跳过
13. **冷却必记** — 必须维护「最近3章使用技巧清单」，用于计算技巧重复度 D 值
...
### v4.1（当前）
- **融合** 冲突质量公式：困境强度 × (wA·A + wB·B + wC·C) / D
- **新增** 权重配置系统：按项目类型（热血/悬疑/情感/轻松）自动设定 A/B/C 权重
- **新增** 技巧池与滑动窗口重复度计数（前3章内），解决技巧枯竭问题
- **新增** 低困境作品替代指标「化解效率」= 困境强度×三要素累加/化解字数
- **新增** 副本矩阵增加「主导冲突类型」标记（外部对抗/内心挣扎/规则突破/均衡型）
- **新增** Step 3.1 强制输出 A/B/C 得分与技巧名称
- **新增** Step 3.4 冲突质量自动计算与合格线校验
- **新增** Step 3.5 关卡6「冲突质量复核」与关卡4增强
- **新增** Step 3.7 冲突质量达标时可豁免技巧重复扣分
- **新增** 冲突质量报告纳入最终结果输出
