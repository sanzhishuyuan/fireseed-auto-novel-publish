...
export const POST = withRoute({ auth: 'none', body: true }, async (request, ctx) => {
  const { code, newUserId } = ctx.body;

  if (!code || !newUserId) {
    return apiError('VALIDATION_REQUIRED', '参数不完整', 400);
  }
  ...
  if (referral.user_id === newUserId) {
    return apiError('VALIDATION_SELF_REFERRAL', '不能使用自己的推广码', 400);
  }
  ...
  if (existingRedemption) {
    return apiError('ALREADY_REDEEMED', '该用户已使用过推广码', 400);
  }
