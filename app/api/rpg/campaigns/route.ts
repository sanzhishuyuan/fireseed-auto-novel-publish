...
/**
 * POST /api/rpg/campaigns — 创建战役（支持世界书关联）
 */
export async function POST(request: NextRequest) {
  try {
    ...
    // 验证世界书所有权（如果指定了 lorebook_id）
    if (lorebook_id) {
      const lb = db.prepare('SELECT id FROM rpg_lorebooks WHERE id = ? AND user_id = ?').get(lorebook_id, user.userId);
      if (!lb) {
