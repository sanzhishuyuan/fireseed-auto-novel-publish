...
export async function POST(
  ...
  try {
    ...
    console.error('[Opp Vote] Error:', error);
    if (error.message?.includes('余额不足')) {
      return apiError('SEED_INSUFFICIENT', 'SEED 余额不足', 402);
    }
    return apiError('INTERNAL_ERROR', '投票失败', 500);
  }
}
...
export async function GET(
  ...
  try {
    ...
    console.error('[Opp Vote] GET error:', error);
    return apiError('INTERNAL_ERROR', '获取投票失败', 500);
  }
}
